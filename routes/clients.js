...
router.post('/', verifyAdmin, upload.fields([
  ...
  try {
    ...
    const existTel = await Client.findOne({ telephone });
    if (existTel) return res.status(400).json({ error: 'Téléphone déjà utilisé' });

    // Password = téléphone hashé
    const hashedPassword = await bcrypt.hash(telephone, 10);

    // URLs Cloudinary
    const photoProfil = req.files['photoProfil']? req.files['photoProfil'][0].path : null;
    const carteRecto = req.files['carteRecto']? req.files['carteRecto'][0].path : null;
    const carteVerso = req.files['carteVerso']? req.files['carteVerso'][0].path : null;
