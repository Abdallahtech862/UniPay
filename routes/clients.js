...
function renderTable(clients){
 ...
 clients.forEach(c=>{
  ...
  const btnBlock=c.bloque?'<button class="action unblock" onclick="toggleBlock(\\''+c._id+'\\',false)">Débloquer</button>':'<button class="action block" onclick="toggleBlock(\\''+c._id+'\\',true)">Bloquer</button>';
    html+=\`<tr><td><div style="display:flex;gap:10px;align-items:center">\${photo}<div><b>${c.prenom||''} \${c.nom||''}</b><br><small style="color:#8a7a60">\${c.pseudo||''}</small></div>
  </div></td><td>\${c.telephone}<br><small>\${c.numeroCNIB||'CNIB: -'}</small></td><td><b>\${(c.solde||0).toLocaleString()} FCFA</b></td><td style="font-size:12px">\${(c.limiteJournaliere||0).toLocaleString()}
<br>\${(c.limiteMensuelle||0).toLocaleString()}</td><td>\${badgeKyc}<div style="margin-top:4px">\${statut}</div></td>
  <td><button class="action view" onclick='voirCNI(\${JSON.stringify(c).replace(/'/g,"&#39;")})'>Voir KYC</button>
  <button class="action limit" onclick="modifierLimites('\${c._id}',${c.limiteJournaliere||0},\${c.limiteMensuelle||0})">Limites</button>
  <br>\${btnBlock}<button class="action" style="background:#fef3c7;color:#92400e" onclick="resetPassword('\${c._id}')">1234</button><button class="action delete" onclick="supprimerClient('\${c._id}')">Supp</button>
  </td></tr>\`;
