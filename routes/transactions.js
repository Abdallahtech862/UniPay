...
<body>
  ...
  <script>
    ...
    function renderTable(transactions){
      ...
      let html = '<table><tr><th>Date</th><th>Type / Op</th><th>Expéditeur</th><th>Destinataire / N°</th><th>Montant</th><th>Statut</th><th class="no-print">Action</th></tr>';
      transactions.forEach(t=>{
        if(!t.expediteur) return;
        const date = t.createdAt ? new Date(t.createdAt).toLocaleString('fr-FR') : '-';
        const type = t.type || (t.montantAnnule?'annulation':'transfert');
        let dest = '-';
        ...
        else if(t.numeroDestination || t.numeroSource) dest = t.numeroDestination || t.numeroSource;
        ...
        // PEUT ANNULER: envoi ou vente qui est encore validee/reussie et pas déjà annulée
        const peutAnnuler = (t.type==='envoi' || t.type==='vente') && (t.status==='validee' || t.status==='reussie') && !t.annulee;
        // Pour compat avec ton ancien model sans type (juste montant)
        const peutAnnuler2 = (!t.type || t.type==='vente' || t.type==='envoi') && t.montant>0 && !t.annulee && (t.status==='validee');

        const actionBtn = peutAnnuler || peutAnnuler2 
          ? \`<button class="btn-danger" style="padding:5px 10px;font-size:11px" onclick="annulerTx('\${t._id}', '\${type}')">Annuler \${type}</button>\`
          : (t.annulee ? '<span class="small">Annulée le '+ (t.dateAnnulation? new Date(t.dateAnnulation).toLocaleDateString('fr-FR'):'') +'</span>' : '-');

        html += \`<tr>
          <td><span class="small">\${date}</span><br><span class="small">\${t._id.slice(-6)}</span></td>
          <td><span class="type type-\${type}">\${type}</span><br><span class="small">\${t.operateur||''}</span><br><span class="small">ID: \${t.depositId? t.depositId.slice(0,8): ''}</span></td>
          <td>\${t.expediteur.prenom} \${t.expediteur.nom}<br><span class="small">\${t.expediteur.telephone}</span><br><span class="small">Solde après: \${(t.soldeExpediteurApres||0).toLocaleString()} F</span></td>
          <td>\${dest} \${t.soldeDestinataireApres!==undefined? '<br><span class="small">Solde dest: '+(t.soldeDestinataireApres||0).toLocaleString()+' F</span>':''}</td>
          <td class="montant">\${(t.montant||0).toLocaleString()} F<br><span class="small">Frais: \${t.frais||0} F</span> \${t.montantAnnule? '<br><span class="small" style="color:red">Annulé: '+t.montantAnnule+'</span>':''}</td>
          <td>\${badgeStatut}</td>
    ...
    function exportCSV(){
      ...
      let csv = 'Date,Type,Operateur,Expediteur,TelExp,Destinataire,Numero,Montant,Frais,Statut,Motif\\n';
      currentTransactions.forEach(t=>{
        const date = new Date(t.createdAt).toLocaleString('fr-FR');
        const dest = t.destinataire? (t.destinataire.prenom+' '+t.destinataire.nom) : (t.numeroDestination||t.numeroSource||'');
        const telDest = t.destinataire? t.destinataire.telephone : '';
        csv += \`"\${date}","\${t.type||''}","\${t.operateur||''}","\${t.expediteur.prenom} \${t.expediteur.nom}","\${t.expediteur.telephone}","\${dest}","\${telDest}",\${t.montant},\${t.frais||0},"\${t.status}","\${(t.motif||'').replace(/"/g,'')}"\\n\`;
      });
      const blob = new Blob([csv], {type:'text/csv;charset=utf-8;'});
      const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='unipay_\${new Date().toISOString().slice(0,10)}.csv'; a.click();
    }
